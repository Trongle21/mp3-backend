...
function fileFilter(_req, file, cb) {
  ...
  if (!ALLOWED_EXT.includes(ext)) {
    return cb(new Error(`Unsupported file extension ${ext}. Allowed: ${ALLOWED_EXT.join(', ')}`));
  }
...
function imageFileFilter(_req, file, cb) {
  ...
  if (!ALLOWED_IMAGE_EXT.includes(ext)) {
    const err = new Error(`Unsupported image extension ${ext}. Allowed: ${ALLOWED_IMAGE_EXT.join(', ')}`);
    err.status = 400;
    return cb(err);
  }
